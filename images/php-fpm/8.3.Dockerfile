...
ENV NEWRELIC_VERSION=12.6.0.34
RUN mkdir -p /tmp/newrelic && cd /tmp/newrelic \
    ...
    && sed -i -e "s/;newrelic.transaction_tracer.max_segments_web = .*/newrelic.transaction_tracer.max_segments_web = \${NEWRELIC_MAX_SEGMENTS_WEB:-0}/" /usr/local/etc/php/conf.d/newrelic.ini \
