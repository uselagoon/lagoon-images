...
if [ "$1" = 'mariadbd' -a -z "$wantHelp" ]; then
  ...
  if [ -d ${MARIADB_DATA_DIR:-/var/lib/mysql}/mysql ]; then
    ...
    /usr/sbin/mariadbd --skip-networking --wsrep_on=OFF &
    pid="$!"
    echo "pid is $pid"

    for i in $(seq 0 $MARIADB_INIT_WAIT_SECONDS); do
      if echo 'SELECT 1' | mariadb -u root; then
        break
      fi
    ...
    mariadb-install-db --skip-name-resolve --skip-test-db --auth-root-authentication-method=normal --datadir=${MARIADB_DATA_DIR:-/var/lib/mysql} --basedir=/usr
...
EOF
    ...
    echo "socket=/run/mariadb/mysqld.sock" >> ${MARIADB_DATA_DIR:-/var/lib/mysql}/.my.cnf
    echo "[mysql]" >> ${MARIADB_DATA_DIR:-/var/lib/mysql}/.my.cnf
    echo "database=${MARIADB_DATABASE}" >> ${MARIADB_DATA_DIR:-/var/lib/mysql}/.my.cnf
    echo "[mysqld]" >> ${MARIADB_DATA_DIR:-/var/lib/mysql}/.my.cnf
    echo "socket=/run/mariadb/mysqld.sock" >> ${MARIADB_DATA_DIR:-/var/lib/mysql}/.my.cnf
    ...
    for f in /docker-entrypoint-initdb.d/*; do
      if [ -e "$f" ]; then
        case "$f" in
          ...
          *.sql)    echo "$0: running $f"; cat $f| envsubst | tee | mariadb -u root -p${MARIADB_ROOT_PASSWORD}; echo ;;
          *)        echo "$0: ignoring $f" ;;
        esac
      fi
    done
