...
RUN apk update \
    ...
    && pie install imagick/imagick:3.8.0 \
    && pie install phpredis/phpredis:6.2.0 \
    && pie install xdebug/xdebug:3.4.7 \
        ## waiting on https://github.com/php/pecl-file_formats-yaml/issues/96
    && pie install pecl/yaml:dev-php7 \
