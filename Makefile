...
docker_build = docker buildx build $(DOCKER_BUILD_PARAMS) --platform linux/amd64,linux/arm64/v8 --cache-from=type=registry,ref=testlagoon/$(1):multiarch --push --build-arg LAGOON_VERSION=$(LAGOON_VERSION) --build-arg IMAGE_REPO=localhost:5000/$(CI_BUILD_TAG) -t localhost:5000/$(CI_BUILD_TAG)/$(1) -t testlagoon/$(1):multiarch -f $(2) $(3)
