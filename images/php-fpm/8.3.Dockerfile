...
FROM php:8.3.33-fpm-alpine3.23
...
COPY --from=docker.io/mlocati/php-extension-installer:2.12 /usr/bin/install-php-extensions /usr/local/bin/
COPY --from=ghcr.io/php/pie:1.5.0-bin /pie /usr/local/bin/
...
ENV NEWRELIC_VERSION=12.11.0.40
