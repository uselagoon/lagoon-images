...
docker compose exec -T mariadb-11-4 sh -c "mysql -D lagoon -u lagoon --password=lagoon < /tmp/showglobalvars.sql" | grep "performance_schema" | grep "ON"
docker compose exec -T mariadb-11-4 sh -c "mysql -D lagoon -u lagoon --password=lagoon < /tmp/showglobalvars.sql" | grep "slow_query_log" | grep "ON"
docker compose exec -T mariadb-11-4 sh -c "mysql -D lagoon -u lagoon --password=lagoon < /tmp/showglobalvars.sql" | grep "long_query_time" | grep "30"
docker compose exec -T mariadb-11-4 sh -c "mysql -D lagoon -u lagoon --password=lagoon < /tmp/showglobalvars.sql" | grep "log_slow_rate_limit" | grep "5"
...
docker compose exec -T postgres-12 bash -c "echo U0VMRUNUIHZlcnNpb24oKTs= | base64 -d > /tmp/selectversion.sql"
docker compose exec -T postgres-12 bash -c "psql -U lagoon -d lagoon < /tmp/selectversion.sql" | grep "PostgreSQL" | grep "12."

# postgres-12 should have lagoon database
docker compose exec -T postgres-12 bash -c "echo XGwrIGxhZ29vbg== | base64 -d > /tmp/listlagoon.sql"
docker compose exec -T postgres-12 bash -c "psql -U lagoon -d lagoon < /tmp/listlagoon.sql" | grep "lagoon"
