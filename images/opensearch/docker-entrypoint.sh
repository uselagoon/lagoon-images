...
if [ -z "$POD_NAMESPACE" ]; then
  ...
  sed -i 's/cluster.initial_master_nodes:.*//' /usr/share/opensearch/config/opensearch.yml
  echo "cluster.initial_master_nodes: ${HOSTNAME}" >> /usr/share/opensearch/config/opensearch.yml
else
  ...
  echo "cluster.initial_master_nodes: ${K8S_SVC_NAME}-0" >> /usr/share/opensearch/config/opensearch.yml
fi
